...
// 回复下方那一行「🧠 记住了：不接受塑料材质 ✕」。
//
// 这是整个记忆透明度设计的落点：回合后抽取在后台自动学、自动写（不弹确认框——收尾这一刻用户的
...
export function LearnedPrefsBar({ userId, prefs, onForget }: LearnedPrefsBarProps) {
  // 已撤销的 key：本地即时隐藏，不等重拉（这一行的价值就在「点了立刻消失」）。
  ...
  const shown = prefs.filter((p) => !forgotten.has(p.key));
  ...
  return (
    <div className="learned-bar">
      ...
      {shown.map((p) => (
        <span key={p.key} className="learned-chip">
          {p.content}
          <button
            className="learned-forget"
            title="别记这条"
            onClick={() => void forget(p.key)}
          >
            ✕
          </button>
        </span>
