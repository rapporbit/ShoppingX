...
PARAMS: tuple[Param, ...] = (
    ...
    Param(
        ...
        maximum=32,
        reload=_LLM,
        help="同时在飞的模型请求数（主 Agent 与各 worker 共用一个池，按 API key 的 RPM 调）。",
    ),
    Param(
        ...
        maximum=10.0,
        reload=_LLM,
        help="两次请求发出时刻的最小间隔（起点到起点，与每条跑多久无关）。0 = 不限速。",
    ),
    Param(
        ...
        reload=_LLM,
        allow_empty=True,
        help="主模型重试用尽后接手。留空或与主模型同名 = 不启用（自己当自己的备用无意义）。",
    ),
    # ---------------- 检索召回 ----------------
    Param(
