...
from app.tools._candidates import hydrate, load_candidates, registry_snapshot
...
def _hydrate_for_thread(thread_id: str, uid: str) -> Any:
    """给 HTTP 入口用的候选 hydrate：进该 thread 的作用域，登记表为空（进程重启过）就从
    ``candidates.json`` 读回——表单点「生成确认单」时任务早已结束，ContextVar 里什么都没有。"""
    session_dir = _safe_session_dir(OUTPUT_ROOT, thread_id)

    def _hydrate(ids: list[str]) -> list[Any]:
        with thread_scope(thread_id, session_dir, uid):
            if not registry_snapshot():
                load_candidates(session_dir)
            return hydrate(ids)
