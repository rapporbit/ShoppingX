...
#
# ⚠️ **待办：qwen3.6-flash 的真实费率还没核实**（百炼的模型页不展示单价，没查到就没往下写——
# 编一个数比空着更坏，成本闸与 credit 配额都吃这张表）。当前它落进默认档（deepseek-v4-flash
# 的价）。偏差范围可控但不为零：planner 约占一次任务 input 的 4%、output 的 18%，若两者单价
# 差一倍，总成本误差 <10%。核实到真实费率后在这里按名补一行即可。
