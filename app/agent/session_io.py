...
一条教训钉在这里：``render_platform_block`` 曾指挥模型调用一个已被取代的派发工具名。
**给模型的动机文本必须跟着工具表走**，否则它照着念一个不存在的名字，只能撞 tool-not-found
白烧一轮。（同型缺陷 2026-09-12 又在 `sentinels.INTERNAL_MARKERS` 和收线通告文案里各抓到一处。）
"""
