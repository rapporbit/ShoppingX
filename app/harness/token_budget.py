...
#
# **默认档钉的是本项目实际在用的主模型**：`deepseek-v4.1-flash`（经 DashScope / 百炼调用）。
# 填的是 **v4 的公开价** ¥1/¥2/¥0.02 每百万（= $0.14 / $0.28 / $0.0028，汇率口径 1 USD =
# 7.14 CNY）——⚠️ v4 在控制台已下架、只剩 v4.1，而百炼没给 v4.1 的单价页；编一个数比沿用一个
# 有出处的旧价更坏。查到真价即就地改。
#
# **别再拿「qwen3.8-flash 单价更低」去换主模型——2026-09-20 实测换过，5 倍贵，已回滚。**
# 同一份代码、同一条 q01_travel_set、只换主 loop 模型：
#   deepseek-v4.1-flash： 5 次模型调用 /  41.4k 输入 / 1219 输出 → $0.0031
#   qwen3.8-flash：      23 次模型调用 / 236.2k 输入 / 4081 输出 → $0.0156
# 单价确实是 qwen 低（输入 -20%），但它在主 loop 里不收敛、跑了 4.6 倍的调用，单价优势被吃光还
# 倒欠；而且它贵的恰好是本项目最吃的那档（缓存命中 ×5，多轮追问约 75% 的输入走它）。
# qwen3.8-flash 仍是 planner 的选型（链路外一次性调用，不进这个循环），价见下面 _PRICES。
#
...
#
# 曾经这张表可以是空的：进入本模块计费的只有主链路，而主链路全走同一个 `LLM_MAIN`（快档只是
# 关了 thinking，同模型同价）；judge 只在离线评测里跑，不经过 run_agent，压根不进这张表。
#
# **那个前提在 2026-09-09 破了**：`LLM_PLANNER` 让 planner 单独换了模型（qwen3.6-flash），
# 于是「一条链上跑两种价位的模型」成了现实——正是上面那句「真到了那天再按名分价」说的那天。
#
# 表里这一行是 **planner 档**（`LLM_PLANNER=dashscope/qwen3.8-flash`，2026-09-09 单独换的模型）：
# 百炼标价 ¥0.8 / ¥2.7 / ¥0.1 每百万 → 0.112 / 0.378 / 0.014。它与主模型的价差落在三档的不同
# 方向上（输入更便宜、输出更贵、缓存命中贵 5 倍），共用默认档会把两边都算错。
#
# 键取 `qwen3.8` 而不是 `qwen`：`LLM_JUDGE` / `LLM_VISION` 是 qwen3.5-flash，**费率没核实**
# （百炼没给它的单价页），让它继续落默认档——宁可用一个有出处的邻近价，也不拿 3.8 的价去冒充 3.5。
# judge 只在离线评测跑、不进这张表；vision 只在用户附图时走一次，偏差影响面小。
_PRICES: dict[str, _PriceTuple] = {
    "qwen3.8": (0.112, 0.378, 0.014),
}
