...
def _load_params() -> None:
    ...
    #   自训 r6（分级CE+BCE）      + 0.15  →  误杀 20.5% / 挡住 52.9%
    #                              + 0.20  →  误杀 30.2% / 挡住 71.0%   ← 切 r6 建议值
    #                              + 0.25  →  误杀 39.8% / 挡住 81.8%
    #   自训 r3（纯分级CE）        + 0.20  →  误杀  3.9% / 挡住 20.7%   ← 门几乎失效
    #
    # 两条要记住的：① **纯 listwise 训练会破坏绝对分数校准**（r3 分布从跨满 0~1 压到
    # .17~.85，真实候选低于 .2 的比例从 58.9% 掉到 11.7%，门等于没开）；联合 pointwise BCE
    # 能把校准拉回来（r6），且排序不降反升。② 即便如此，**自训模型的绝对判别力仍不如原版**
    # ——同等误杀率下 r6 只挡住 81.8%、原版挡住 88.6%。排序腿和判别腿要的东西不一样，
    # 训练只优化了前者。切 r6 时门会略松一点，靠它更强的排序把漏网的压下去。
