...
def compact_candidates(
    ...
    **决定把哪些 item_id 传下去** + 给用户讲人话——凡是不服务这两件事的字段，都是纯烧 token。
    ...
    ``title_chars`` 供**回显场景**截短标题：仅当这批候选的完整标题确实已在上下文里出现过才可传
    （电商长标题实测 200+ 字符，重复即浪费）。**首次呈现**（item_search）不要传：模型要靠完整
    标题判断召回质量。item_picker 的 picks **也不传**——picks 来自整池，模型只见过渲染头部几条，
    截短会让它为没见过全名的商品写理由（A0-3 实测因此白搜一轮）。
