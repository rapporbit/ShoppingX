...
async def _client(monkeypatch: pytest.MonkeyPatch, tmp_path):  # type: ignore[no-untyped-def]
    """开着鉴权的 ASGI 客户端，输出根钉到 tmp（表单接口要从 candidates.json 读候选）。"""
...
async def test_http_prepare_list_resolve_roundtrip(_client) -> None:  # type: ignore[no-untyped-def]
    """表单 → 出卡（候选从落盘的 candidates.json 读回）→ 列表能看到 → 同意落单 → 别人碰不到。"""
    import json
    ...
    (sd / "candidates.json").write_text(
        json.dumps([POOL["B01"].model_dump(mode="json")]), encoding="utf-8"
    )
