...
（格式：族 —— 历史 —— 为什么 prompt 修不好）

- `accessory_flood` —— 「搜手机出配件」 —— 根因是数据不是算法：整机与配件在源数据里
  同类目（TCL 标成 Nintendo DS）。黑名单闸 + 品类过滤两方案均被实测证伪
- `usage_confusion` —— 篮球包 vs 通勤包 —— 展示门只挡跨品类垃圾，挡不住同品类用途混淆
  （cross-encoder 0.97 交叠）
- `numeric_spec` —— 16 寸笔记本包 —— 数值是 embedding 处理不了的算子，只能靠 planner
  把它抽成硬约束走专道
...
FAMILIES: dict[str, dict] = {
    "accessory_flood": {
        "weight": 18,
        "cat": re.compile(
            r"phone|camera|laptop|computer|console|printer|headphone|tablet|tv|watch"
        ),
        "seeds": [
            "想买手机，安卓系统，拍照好的",
            "我要买 HP 打印机的墨盒，对比几个平台哪个划算",
            "想买第3代的苹果无线耳机，预算200美元",
        ],
        "hint": "用户要的是**整机/正品本体**，但这个品类的配件（壳、膜、线、耗材、支架）"
        "在库里数量碾压本体。造的 query 要像真人一样只说品类不强调「整机」——"
        "正因为不强调，模型才容易召回一堆配件。",
    },
    "usage_confusion": {
        "weight": 14,
        ...
        "seeds": [
            "想买个通勤双肩包，预算 80 美元，要能装 16 寸笔记本",
            "帮我找一个便宜耐用的旅行背包，防水尼龙材质，不要塑料的，预算 50 美元",
        ],
    ...
    "numeric_spec": {
        "weight": 14,
        "cat": re.compile(
            r"laptop|computer|monitor|storage|drive|memory|bottle|clothing|shoes|tv|battery|luggage|backpack"
        ),
        "seeds": [
            "要能装17寸以上笔记本的双肩包，预算60美元",
            "想要一个256GB的U盘，传文件用，预算40美元",
            "最好能保温12小时以上",
            "想买一件男士纯棉短袖T恤，要M码的，预算30美元",
        ],
        "hint": "必须带一个**可比较的数值规格**（尺寸/容量/时长/码数/功率），"
        "且数值要是该品类真实存在的档位。这类约束 embedding 抓不住，只能靠 planner 抽成硬约束。",
    },
    "domain_drift": {
        "weight": 14,
        "cat": re.compile(r".*"),
        "seeds": [
            ...
            "给新家配齐厨房好物，预算800美元",
            "新生入学一套，预算 1500",
            "想把全套护肤品换成成分党友好的，预算500美元，敏感肌能用的",
        ],
        "hint": "**不直接说品类**，只给场合/身份/人生阶段，让模型自己推品类域。"
        "这正是 planner 判错域的高发形态。",
    },
    "pollution_backfill": {
        "weight": 10,
        "cat": re.compile(r"watch|phone|shoes|bag|camera|headphone"),
        "seeds": [
            "给我推荐几款相机，要求日本品牌，主要拍人像照片，预算 10000",
            "帮我找点便宜的大牌平替包，越像越好",
        ],
        "hint": "带**品牌或价位锚**的 query——召回池容易被同品牌的其它品类灌满"
        "（搜某牌相机返回该牌背带）。",
    },
    "cross_lingual": {
        "weight": 10,
        "cat": re.compile(r".*"),
        "seeds": [
            "我要一套床上三件套，纯棉的，预算 1000，要有东方元素（如刺绣、花鸟纹样）",
            "想要更明显的东方元素（如刺绣、花鸟纹样）",
        ],
        "hint": "用**地道中文说法**（含中式风格词、方言化叫法、网络流行叫法），"
        "商品库里全是英文标题，字面对不上，只能靠向量跨语言对齐。",
    },
    "gender_age": {
        "weight": 10,
        ...
        "seeds": [
            "想买条男士运动短裤，便宜点能跑步穿的",
            "帮我挑几件夏天穿的男士短袖，预算 40 美元以内",
            "送闺蜜的伴手礼，预算100美元，要可爱有质感",
        ],
    ...
    "retract": {
        ...
        "hint": "**必须是两轮**：第 1 轮立一个硬约束，第 2 轮明确撤回或放宽它。"
        "考的是 P_t 能否把旧约束真正拿掉。",
    },
}
...
def _load_cards() -> dict[str, dict]:
    ...
    for line in CARDS_PATH.open(encoding="utf-8"):
        ...
        d = json.loads(line)
        c = cards.setdefault(
            d["category"], {"category": d["category"], "aliases": [], "prices": []}
        )
        c["aliases"] = list(
            dict.fromkeys(
        ...
        c["prices"] += [
            float(m.group(1))
            for e in d.get("raw_evidence", [])
            if (m := re.search(r"\$([\d.]+)", e))
        ]
    for c in cards.values():
        c["median_price"] = (
            round(sorted(c["prices"])[len(c["prices"]) // 2], 2) if c["prices"] else None
        )
    return cards
...
async def main() -> None:
    ...
    n_planned = sum(p[2] for p in plans)
    print(f"{len(FAMILIES)} 个 bad case 族，排定 {len(plans)} 次请求 / 约 {n_planned} 条")
                        ...
                        "turns": [
                            {
                                ...
                                "followup_type": (
                                    "retract_constraint"
                                    if i and family == "retract"
                                    else ("add_constraint" if i else None)
                                ),
                            }
