...
def main() -> None:
    ...
    report = {
        ...
        "reward 首段→末段": (
            round(reward_seg[-1] - reward_seg[0], 4) if len(reward_seg) >= 2 else None
        ),
        ...
        "completion 长度均值": round(
            statistics.mean(
                [r["completions/mean_length"] for r in train if "completions/mean_length" in r]
            ),
            1,
        )
        if train
        ...
        "截断率均值": round(
            statistics.mean([r.get("completions/clipped_ratio", 0.0) for r in train]), 4
        )
        if train
        else None,
        "eval 序列": [
            {
                ...
                "reward": round(r["eval_reward"], 4),
                "kl": round(r.get("eval_kl", 0), 4),
                "零方差组比例": r.get("eval_frac_reward_zero_std"),
            }
            for r in evals
        ],
        "单步耗时中位 s": round(
            statistics.median([r["step_time"] for r in train if "step_time" in r]), 2
        )
        if train
        else None,
    }
